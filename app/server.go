...
func replicateMaster(address string, store *Store) {
	...
	for {
		...
		commands := parse(buff[:n])
		fmt.Printf("print::: %T, %v", commands, commands)
		...
		for _, command := range commands {
			...
			case "set":
				if len(commandParts) >= 3 {
					ttl := time.Duration(0)
					if len(commandParts) == 5 && commandParts[3] == "px" {
						if parsedTTL, err := strconv.Atoi(commandParts[4]); err == nil {
							ttl = time.Duration(parsedTTL) * time.Millisecond
						}
					}
					store.Set(commandParts[1], commandParts[2], ttl)
